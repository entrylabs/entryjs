...
p.createHandle = function() {
    ...
    rotateKnob.on("mouseup", function(e) {
        handle.dispatchEditEndEvent();
    });
    ...
    directionArrow.graphics.ss(4, 1, 1).s(this.arrowColor).f(this.arrowColor)
        .dc(0,0,this.DHANDLE_RADIUS)
        .mt(0,0).lt(0,-40).lt(7,-32).lt(-7,-32).lt(0,-40).es();
    ...
    directionArrow.on("mouseup", function(e) {
        handle.dispatchEditEndEvent();
    });
    ...
    centerPoint.on("mouseup", function(e) {
        handle.dispatchEditEndEvent();
    });
    ...
    for (var i = 0; i < 8; i++) {
        ...
        knob.graphics.beginFill(this.color)
                     .ss(1, 2, 0)
                     .s(this.color)
                     .dr(-3,-3,6,6);
        knob.knobIndex = i;
        ...
        knob.on("mousedown", function(e) {
            var otherKnobIndex = this.knobIndex + 4 > 7 ?
                this.knobIndex + 4 - 8 : this.knobIndex + 4;
...
p.checkCenterPointState = function(x,y){
    ...
    if(res > standard && Entry.engine.isState('stop')) {
        Entry.toast.warning('이런! 조심하세요!','오브젝트의 중심이 이미지에서 크게 벗어나서 원위치로 되돌렸습니다.');
        return true;
    }
    return false;
}
...
p.renderEdge = function() {
    ...
    this.edge.graphics.clear().ss(10, 2, 0).s("rgba(254,254,254,0.01)")
        .lt(-width/2,-height/2)
        .lt(0,-height/2)
        .lt(0,-height/2)
        .lt(+width/2,-height/2)
...
p.renderRotateKnob = function() {
    var width = this.width;
    var height = this.height;
    this.rotateKnob.graphics.clear()
        .ss(1, 2, 0)
...
p.renderBorder = function() {
    var width = this.width;
    var height = this.height;
    this.border.graphics.clear().ss(1, 2, 0).s(this.color)
        .lt(-width/2,-height/2)
        .lt(0,-height/2)
        .lt(0,-height/2)
        .lt(+width/2,-height/2)
...
p.getLocalCoordinate = function(pos) {
    ...
    pos.y -= this.y;
    return {
        x: pos.x * Math.cos(rotation) +
           pos.y * Math.sin(rotation),
        y: pos.y * Math.cos(rotation) -
