...
Blockly.createDom_ = function(container) {
  ...
  */
  ...
  for (var i = 1; i < 5; i++) {
    blockPattern = Blockly.createSvgElement('pattern',
        {'id': 'blockPattern' + i, 'patternUnits': 'userSpaceOnUse',
         'width': 125, 'height': 33, 'x': 0, 'y': 0}, defs);
    blockImg = Blockly.createSvgElement('image',
  ...
  if (!Blockly.readOnly) {
    ...
    if (Blockly.hasCategories) {
      ...
      if (Blockly.hasBlockMenu) {
        ...
        Blockly.bindEvent_(blockMenu.menuView_, 'wheel', blockMenu, Blockly.onMouseWheelFlyout_);
