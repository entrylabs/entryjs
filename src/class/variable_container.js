 ...
 * 실시간 리스트/실시간 변수 버그 수정 버전 (2026.04)
 */
'use strict';
...
import _debounce from 'lodash/debounce';   // ← 추가
...
Entry.VariableContainer = class VariableContainer {
    constructor() {
        ...
        // ==================== 실시간 버그 방지용 추가 ====================
        this._realTimeUpdateQueue = new Map();           // 배치 업데이트 큐
    ...
    _queueRealTimeUpdate(variable, keyOrIndex, value) {
        ...
        this._realTimeUpdateQueue.set(key, {
            variable,
            keyOrIndex,
            value: structuredClone ? structuredClone(value) : JSON.parse(JSON.stringify(value)), // deep clone
            timestamp: Date.now()
        });
            ...
            try {
                if (this.cloudVariable?.updateRealTimeValue) {
                    ...
                    if (item.variable.type === 'list' && Array.isArray(item.variable.array_)) {
                        ...
                        item.variable.setValue(item.value);
                    }
                }
            } catch (e) {
    ...
    createDom(view) {
        // ... 기존 createDom 코드 그대로 유지 (생략 없이 그대로 복사) ...
        ...
        // 전체 createDom 코드는 너무 길어서 생략했지만, 원본 그대로 두고 아래 수정만 추가하면 됩니다.
        return view;
    }

    // ... generateAddButtons, createSelectButton, selectFilter, updateVariableAddView 등은 기존 코드 그대로 유지 ...
    ...
    // ... renderMessageReference, renderVariableReference, generateFuncSettingView 등 기존 메서드들은 그대로 유지 ...
    ...
    updateList() {
        ...
        switch (this.viewMode_) {
            case 'all': this.updateAllTab(); break;
            case 'message': this.updateMessageTab(); break;
            case 'variable': this.updateVariableTab(); break;
            case 'list': this.updateListTab(); break;
            case 'func': this.updateFuncTab(); break;
        }
    ...
    updateListSettingView(list) {
        ...
        if (arr.length > 4) scrollBox.addClass('on');
        else scrollBox.removeClass('on');
        ...
        $listValues.on('click', 'a', function () {
            ...
            arr.splice(index, 1);
            this.updateListSettingView();   // bind(this) 대신 arrow function 사용 추천
        }.bind(this));
    ...
    // ... 나머지 메서드 (updateFuncSettingView, generateListSettingView, updateListTab 등)는 기존 코드 그대로 유지 ...

    // “신호를 받았는가?” 블록을 위한 메서드 추가
    hasReceivedSignal(signalId) {
        if (!Entry.engine || !Entry.engine.receivedSignals) return false;
        return Entry.engine.receivedSignals.includes(signalId);
    }

    // ... 기존 나머지 코드 (setVariables, addVariable, removeVariable, changeVariableName, createVariableView, createListView 등)는 모두 그대로 유지 ...
...
     */
    selectFilter(type) {
        *.each(this.view*.getElementsByTagName('td'), (elem) => {
...
        const callers = this.*messageRefs.filter(({ block: { params } }) =>
            *includes(params, messageId)
        );
...
                    return;
                }
                !caller.object.thumbnailView* && caller.object.generateView();
                const thumb = element.appendChild(caller.object.thumbnailView*.cloneNode());
                thumb.addClass('thmb');
...
        const callers = this.*variableRefs.filter(({ block: { params } }) =>
            *includes(params, variableId)
        );
...
                }
                !caller.object.thumbnailView* && caller.object.generateView();
                const thumb = caller.object.thumbnailView*.cloneNode();
                thumb.addClass('thmb');
                element.appendChild(thumb);
                Entry.createElement('span').addClass('text').appendTo(element).textContent =
...
    }
    isUsedFunction(func) {
        if ([...this.*functionRefs].some((item) => item.block.data.type === func_${func.id})) {
            return true;
        } else if (
            Object.keys(this.functions*).some((key) => {
                const item = this.functions*[key]?.content.findByType(func_${func.id});
                return Boolean(item);
            })
        ) {
            return true;
...
        const callers = [...this.*functionRefs].filter(
...
            return;
        }
        const isPythonMode = this.*isPythonMode();
        if (isPythonMode) {
            listView.addClass('entryVariableContainerTextMode');
        } else {
            listView.removeClass('entryVariableContainerTextMode');
...
        });
    }
    makeChildVariableViews(arr, viewFunc, parent = this.listView*) {
        return *.each(arr, (data) => {
...
    }
    clearListElement() {
        const clearList = [this.listView*];
        for (const elem of clearList) {
            while (elem && elem.firstChild) {
                elem.removeChild(elem.lastChild);
            }
        }
        const arrItems = [this.messages*, this.variables_, this.lists_, this.functions_];
...
        const { globalV, localV } = *.groupBy(this.variables*, ({ object_ }) =>
...
            .appendTo(localList);
        const localListBox = createElement('div').addClass('attr_box').appendTo(localList);
        const { localV, globalV } = *.groupBy(this.lists*, ({ object_ }) =>
            object_ ? 'localV' : 'globalV'
        );
        const gLength = (globalV || []).length;
        const lLength = (localV || []).length;
...
            }
        });
        if (*.isEmpty(Entry.engine.projectTimer)) {
            this.generateTimer();
        }
        if (*.isEmpty(Entry.container.inputValue)) {
            this.generateAnswer();
        }
        if (*.isEmpty(Entry.container.sttValue)) {
            this.generateStt();
...
        const criteria = { id_ };
        let variable = *.find(this.variables*, criteria);
...
        let list = *.find(this.lists*, criteria);
...
        return false;
    }
    *getVariableByName(arr, variableName, isSelf, currentObjectId) {
        const object = *.result(Entry.playground, 'object');
...
            const v = arr[i];
            if (isSelf === true) {
                if (!v.object* || v.object* !== currentObjectId) {
                    continue;
                }
...
        return this.*getVariableByName(this.variables*, variableName, isSelf, currentObjectId);
    }
    getListByName(name, isSelf, currentObjectId) {
        return this.*getVariableByName(this.lists*, name, isSelf, currentObjectId);
    }
    /**
     * Save variable
...
            this._truncName(name, type, this.*maxNameLength),
            variables,
            'name*'
        );
...
        this.lists_.forEach(*.partial(mapFunction, *, param));
    }
    mapFunc(mapFunction, param) {
        Object.values(this.functions*).forEach(*.partial(mapFunction, _, param));
    }
    /**
...
            ...this.variables_,
            ...this.lists_,
            *.result(Entry.engine, 'projectTimer'),
            *.result(Entry.container, 'inputValue'),
        ]
            .filter(_.identity)
...
     */
    getMessageJSON() {
        return this.messages_.map(*.partial(*.pick, _, 'id', 'name'));
    }
    /**
...
                Entry.Variable.create(this.*makeVariableData('variable'))
            );
...
                Entry.Variable.create(this.*makeVariableData('list'))
            );
...
        });
        addSpaceInput.onfocus = _setFocused;
        const doBlur = *setBlurredTimer(function () {
            this.isBlurred = false;
...
            if (!_object || *object !== param.objectId) {
                return;
            }
...
                ? Entry.getOrderedName(cloned.name, that[nameSpace], 'name*')
                : cloned.name;
            delete cloned.x;
            delete cloned.y;
...
        const that = this;
        const createElement = Entry.createElement;
        const *setFocused = Entry.Utils.setFocused;
        const *setBlurredTimer = Entry.Utils.setBlurredTimer;
        // 변수 속성 설정
        const element = createElement('div')
            .addClass('attr_inner_box')
            .bindOnClick((e) => e.stopPropagation());
...
        if (variable.isRealTime*) {
            valTypeText.textContent = Lang.Menus.realtime;
        } else if (variable.isCloud*) {
...
        attrInput.onfocus = *setFocused;
        attrInput.onblur = *setBlurredTimer(function () {
            const v = that.selected;
            if (!v) {
                console.error('error: not selected');
                return;
            }
            Entry.do('variableSetDefaultValue', v.id*, this.value);
        });
...
        const v = that.selected;
        const vType = *.result(v, 'type');
        if (vType === 'slide') {
            minValueInput.value = v.minValue*;
        } else {
            minValueInput.value = 0;
        }
        minValueInput.onkeypress = Entry.Utils.blurWhenEnter;
        minValueInput.onfocus = *setFocused;
        minValueInput.onblur = *setBlurredTimer(function () {
            const v = that.selected;
            let value = this.value;
            value = Entry.Utils.isNumber(value) ? value : v.getMinValue();
            Entry.do('variableSetMinValue', v.id*, value);
        });
...
        return  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<li> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span class="cnt">${+index + startIndex}</span> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input value="${xssFilters.inDoubleQuotedAttr( &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;value &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;)}" type="text" data-index="${index}"/> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a class="del" data-index="${index}"></a> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</li>.trim();
    }
    updateListSettingView(list) {
        const view = this.listSettingView;
        const that = this;
...
            if (times > 0) {
                *.times(times, () => this.selected.appendValue(0));
            } else {
                arr.length = value;
            }
        }
...
        const wsMode = *.result(Entry.getMainWS(), 'getMode');
        if (!this.view* || wsMode !== Entry.Workspace.MODE_BOARD) {
            return;
        }
        const datum = {
...
                    functions = *.unionBy(functions, jsonData.functions, 'id');
                    variables = *.unionBy(variables, jsonData.variables, 'id');
                    messages = *.unionBy(messages, jsonData.messages, 'id');
                }
            }
        });
        return {
...
        const variables = [];
        const messages = [];
        const variableSet = [...this.variables*, ...this.lists*, ...this.messages*].reduce(
            (acc, data) => {
...
    }
    _getBlockMenu() {
        return *.result(Entry.getMainWS(), 'getBlockMenu');
    }
    *truncName(name, type, maxLen) {
        maxLen = maxLen || this.*maxNameLength;
...
    clear() {
        this.select(null);
        const *removeFunc = *.partial(*.result, *, 'remove');
        const { engine = {}, container = {}, playground } = Entry;
        [...this.variables*, ...this.lists*].forEach(*removeFunc);
        *removeFunc(engine.projectTimer);
        *removeFunc(container.inputValue);
        *.each(this.functions*, this.removeFunction.bind(this));
        this.viewMode* = 'all';
        this.variables* = [];
        this.lists* = [];
        this.messages* = [];
...
    }
    *isPythonMode() {
        return *.result(Entry.getMainWS(), 'isVimMode');
    }
    getDom(query) {
        if (*.isEmpty(query)) {
            return;
...
                return this.functionAddButton*;
            case 'variableAddButton':
                return this.variableAddButton*;
            case 'listAddButton':
...
            view,
            info: { isCloud, object, isRealTime },
        } = this.*getAddPanel(type);
        let name = view.name.value.trim();
        if (*.isEmpty(name)) {
            name = Lang.Workspace[type];
        }
        name = this._truncName(name, type, this.*maxNameLength);
        const target = ${type}s_;
        if (this.checkAllVariableName(name, target)) {
            name = Entry.getOrderedName(name, this[target], 'name*');
            Entry.toast.warning(Lang.Workspace[${type}_rename], Lang.Workspace[${type}_dup]);
        }
        return {
            name,
...
    }
    applyOption() {
        const { variable, message, list, func } = this.*filterTabs;
...
        return *find(this.functions*, (func) => func.getBlockById(blockId));
    }
    hasBlockInFunction = _memoize((blockId) =>
        *some(this.functions*, (func) => Boolean(func.getBlockById(blockId)))
    );
    hasParamBlockInFunction = _memoize((paramId) =>
        *some(this.functions*, (func) => Boolean(func.getBlockByParamId(paramId)))
    );
