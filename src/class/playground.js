...
Entry.Playground = class Playground {
                ...
                (value) => {
                    ...
                    if (!hanjaEnable) {
                        if (/[\u4E00-\u9FFF]/.exec(textValue) != null) {
                            font = options[0][1];
                            window.EntryModal.alert(Lang.Menus.not_supported_text);
                        }
                    }
    ...
    nameViewBlur() {
        ...
        if (this.nameView.value.trim() === '') {
            window.EntryModal.alert(Lang.Workspace.enter_the_name).then(() => {
                this.nameView.focus();
            });
            return true;
        }
        ...
        for (let i = 0; i < nameViewArray.length; i++) {
            if (
                ...
                window.EntryModal.alert(Lang.Workspace.name_already_exists).then(() => {
                    this.nameView.focus();
                });
                return true;
            }
        ...
        function nameViewBlur() {
            ...
            for (let i = 0; i < nameViewArray.length; i++) {
                if (nameViewArray.eq(i).val() == nameView.value && nameViewArray[i] != this) {
                    return window.EntryModal.alert(Lang.Workspace.name_already_exists).then(() => {
                        nameView.focus();
                    });
                }
            }
