...
Entry.Tooltip = class Tooltip {
    constructor(data, opts) {
        this.init(data, opts);
    }

    usedClasses = 'up down left right edge_up edge_down edge_left edge_right';
    ...
    init(data, opts) {
        ...
        this._resizeEventFunc = Entry.Utils.debounce(this.alignTooltips.bind(this, 200));
    ...
    _renderTooltip(data) {
        ...
        tooltipDom.bind('mousedown', (e) => {
            e.stopPropagation();
            setTimeout(() => {
                return Entry.disposeEvent.notify(undefined, true);
            }, 150);
        });
    ...
    restrictAction() {
        const doms = this.data.map(function(d) {
            return d.targetDom;
        });
