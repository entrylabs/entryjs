    ...
    return {
        ko: {
            ...
            Helper: {
                ...
                robolink_codrone_diy_drone_control_headless           : "드론 좌표 기준을 변경합니다.<br><font color='dodgerblue'>방향 고정 켜기</font>를 하면 '이륙 시'와 '방향초기화'를 했을 때 드론이 바라보는 방향이 0도로 고정됩니다. 이 때에는 Yaw를 조작하여 드론이 다른 방향을 보게 하여도 처음 지정한 방향을 기준으로 움직입니다.<br><font color='crimson'>방향 고정 끄기</font>를 선택하면 드론이 바라보는 방향이 0도가 됩니다.",
                robolink_codrone_diy_drone_control_drone_reset_heading: "드론의 방향을 초기화합니다.<br><font color='dodgerblue'>방향 고정 켜기</font> 설정이 되어 있는 경우 이 블럭을 실행하면 드론이 바라보는 방향을 0으로 초기화합니다.",
                robolink_codrone_diy_drone_control_quad_one           : "드론 조종 값을 지정합니다.<br>입력 가능한 값의 범위는 -100 ~ 100입니다. 정지 상태에서 Throttle 값을 50이상으로 지정하면 드론이 이륙합니다. 명령 전달 후 바로 다음 블럭으로 넘어갑니다.",
                robolink_codrone_diy_drone_control_quad_one_delay     : "드론 조종 값을 지정합니다.<br>입력 가능한 값의 범위는 -100 ~ 100입니다. 정지 상태에서 Throttle 값을 50이상으로 지정하면 드론이 이륙합니다. 지정한 시간이 지나면 해당 조종 값을 0으로 변경합니다. 지정한 시간이 끝날 때까지 다음 블럭으로 넘어가지 않습니다.",
                robolink_codrone_diy_drone_control_quad               : "드론 조종 값을 지정합니다.<br>입력 가능한 값의 범위는 -100 ~ 100입니다. 정지 상태에서 Throttle 값을 50이상으로 지정하면 드론이 이륙합니다. 명령 전달 후 바로 다음 블럭으로 넘어갑니다.",
                robolink_codrone_diy_drone_control_quad_delay         : "드론 조종 값을 지정합니다.<br>입력 가능한 값의 범위는 -100 ~ 100입니다. 정지 상태에서 Throttle 값을 50이상으로 지정하면 드론이 이륙합니다. 지정한 시간이 지나면 해당 조종 값을 0으로 변경합니다. 지정한 시간이 끝날 때까지 다음 블럭으로 넘어가지 않습니다.",
                robolink_codrone_diy_drone_control_position_one       : "드론의 방향과 거리, 이동 속도를 지정하여 지정한 위치로 이동합니다.<br>'기다리기'를 선택한 경우 동작이 끝나기까지 충분한 시간을 현재 블럭에서 기다립니다. '넘어가기'를 선택한 경우 블럭 실행 후 바로 다음 블럭으로 넘어갑니다.",
                robolink_codrone_diy_drone_control_position_turn      : "드론의 회전 방향과 각도, 회전 속도를 지정하여 지정한 각도로 회전합니다.<br>'기다리기'를 선택한 경우 동작이 끝나기까지 충분한 시간을 현재 블럭에서 기다립니다. '넘어가기'를 선택한 경우 블럭 실행 후 바로 다음 블럭으로 넘어갑니다.",
                robolink_codrone_diy_drone_control_position_location  : "드론의 X, Y, Z 축의 방향과 거리, 이동 속도를 설정하여 지정한 위치로 이동합니다.<br>'기다리기'를 선택한 경우 동작이 끝나기까지 충분한 시간을 현재 블럭에서 기다립니다. '넘어가기'를 선택한 경우 블럭 실행 후 바로 다음 블럭으로 넘어갑니다.",
                robolink_codrone_diy_drone_motor_stop                 : "모든 모터의 작동을 정지합니다.",
                robolink_codrone_diy_drone_motor_single               : "모터 제어 블럭입니다.<br>모터의 순서는 12시 방향부터 차례대로 1(앞 오른쪽), 2(뒤 오른쪽), 3(뒤 왼쪽), 4(앞 왼쪽) 입니다.<br>모터 회전에 사용 가능한 값의 범위는 0 ~ 4095입니다.",
                robolink_codrone_diy_drone_light_manual_single_off    : "드론의 모든 LED를 끕니다.",
                robolink_codrone_diy_drone_light_manual_single_input  : "드론 LED를 조작하는데 사용합니다.<br>10진수(0 ~ 65535) 또는 16진수(0x00 ~ 0xFFFF) 값을 사용할 수 있습니다.<br>각각의 비트는 LED를 선택하는 스위치 역할을 합니다.<br>밝기 값은 0 ~ 255 사이의 값을 사용할 수 있습니다.<br>값이 커질수록 더 밝아집니다.",
                ...
                robolink_codrone_diy_controller_buzzer_scale          : "지정한 옥타브의 음을 지정한 시간동안 연주하거나 예약합니다.<br>이 블럭에서 연주를 선택할 경우 소리를 즉시 연주하고, 연주를 명령한 시간동안 기다립니다. 예약을 선택한 경우에는 소리를 예약하고, 바로 다음 블럭으로 넘어갑니다. 예약은 최대 12개까지 누적할 수 있습니다.",
                robolink_codrone_diy_controller_buzzer_hz             : "지정한 주파수의 소리를 지정한 시간동안 연주하거나 예약합니다.<br>권장 사용 범위는 250 ~ 8000 입니다.<br>4옥타브를 기준으로 도(261), 도#(277), 레(293), 레#(311), 미(329), 파(349), 파#(370), 솔(392), 솔#(415), 라(440), 라#(466), 시(493)입니다. 여기에서 한 옥타브를 올라갈 때마다 주파수 값이 두 배가 됩니다. 한 옥타브를 내려갈 때에는 주파수 값이 절반이 됩니다. 예를 들면 3옥타브의 도는 130.8128Hz, 4옥타브의 도는 261.6256Hz, 5옥타브의 도는 523.2511Hz 입니다. 이 블럭에서 연주를 선택할 경우 소리를 즉시 연주하고, 연주를 명령한 시간동안 기다립니다. 예약을 선택한 경우에는 소리를 예약하고, 바로 다음 블럭으로 넘어갑니다. 예약은 최대 12개까지 누적할 수 있습니다.",
            },
        },
