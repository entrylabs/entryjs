...
class mechatro {
    constructor() {
        ...
        this.prev_sensor_data = { '2': 0, '4': 0, '6': 0, '7': 0, '10': 0 };

        this.eventCheck = {
            FALLING: { '2': false, '4': false, '6': false, '7': false, '10': false, 'TRIGGER': false },
            RISING: { '2': false, '4': false, '6': false, '7': false, '10': false, 'TRIGGER': false },
        }
    ...
    afterReceive(pd) {
        ...
        Object.keys(this.prev_sensor_data).forEach((key) => {
            if (this.prev_sensor_data[key] ^ pd[key]) {
                if (pd[key]) {
                    this.eventCheck.RISING[key] = true;
                    this.eventCheck.RISING.TRIGGER = true;
                } else {
                    this.eventCheck.FALLING[key] = true;
                    this.eventCheck.FALLING.TRIGGER = true;
                }
            }
            this.prev_sensor_data[key] = pd[key];
        });

        if (this.eventCheck.RISING.TRIGGER) {
            this.eventCheck.RISING.TRIGGER = false;
            Entry.engine.fireEvent('event_digital_input_is_rising');
        }

        if (this.eventCheck.FALLING.TRIGGER) {
            this.eventCheck.FALLING.TRIGGER = false;
        ...
        return {
            mechatro_event_rising: {
                ...
                func(sprite, script) {
                    ...
                    if (Entry.mechatro.eventCheck.RISING[portNo]) {
                        Entry.mechatro.eventCheck.RISING[portNo] = false;
            ...
            mechatro_event_falling: {
                ...
                func(sprite, script) {
                    const portNo = script.getNumberField('PORT', script);
                    if (Entry.mechatro.eventCheck.FALLING[portNo]) {
                        Entry.mechatro.eventCheck.FALLING[portNo] = false;
                        return script.callReturn();
                    }
                    return this.die();
                },
